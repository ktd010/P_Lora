...
class LoRAParametrization(nn.Module):
    def __init__(self, fan_in, fan_out, fan_in_fan_out=False, rank=4, lora_dropout_p=0.0, lora_alpha=1):
        ...
        # Store lora_dropout_p as an attribute
        self.lora_dropout_p = lora_dropout_p
        
        # Remove any hardcoded device assignment
    ...
    def lora_forward(self, X):
        ...
        dropout_mask = self.lora_dropout_mask.to(X.device)
        
        # Apply dropout if needed
        dropout_A = self._dropout(lora_A) if self.lora_dropout_p > 0 else lora_A
        
        return X + torch.matmul(*self.swap((lora_B, dropout_A))).view(X.shape) * self.scaling
